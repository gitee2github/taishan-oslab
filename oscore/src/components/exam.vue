  ...
  methods: {
    ...
    fetch_data: function() {
      ...
      this.$http.get('/json/scores').then(response => {
        ...
        this.columns.forEach(function(item) {
          if (item.title.startsWith('lab') && item.isExam === true) {
    ...
    calculate_tableData: function() {
      ...
      this.columns.forEach(function(name) {
        if (name.title.startsWith('lab') && name.isExam === true) {
          var tmp = {
            'title': name.title,
            'G010': 0,
            'G050': 0,
          ...
          self.selected_students.forEach(function(item) {
            ...
            if (item[tmp.title] >= 10) {
              tmp.G010 += 1
            }
            if (item[tmp.title] >= 50) {
              tmp.G050 += 1
            }
            if (item[tmp.title] >= 60) {
              tmp.G060 += 1
            }
            if (item[tmp.title] >= 70) {
              tmp.G070 += 1
            }
            if (item[tmp.title] >= 80) {
              tmp.G080 += 1
            }
            if (item[tmp.title] >= 90) {
              tmp.G090 += 1
            }
            if (item[tmp.title] >= 100) {
              tmp.G100 += 1
            }
          })
          self.tableData.push({
